...
type
  ...
  TIdSecByteArray = record
    Length: TIdC_UINT;
    Data: PByte;
  end;

  TIdSecReadStatus = (sslDataAvailable, sslNoData, sslEOF, sslUnrecoverableError);

  TIdSecSocket = class;

  // TIdSecIOHandlerSocketOpenSSL and TIdServerIOHandlerSSLOpenSSL have some common
  // functions, but they do not have a common ancestor, so this interface helps
  // bridge the gap...
  IIdSecOpenSSLCallbackHelper = interface(IInterface)
  ...
  TIdSecCipher = class(TPersistent)
  protected
  ...
  public
    constructor Create(AOwner: TIdSecSocket);
  published
  ...
  TIdSecContext = class(TPersistent)
  private
...
//    fPasswordRoutineOn: Boolean;
  ...
  public
    ...
    function GetCallbackHelper: IIdSecOpenSSLCallbackHelper;
...
//    property PasswordRoutineOn: Boolean read fPasswordRoutineOn write fPasswordRoutineOn;
  ...
  published
    property SSLVersions : TIdSecVersions read fSSLVersions write fSSLVersions;
    property Method: TIdSecVersion read fMethod write fMethod;
...
//    property VerifyFile: String read fVerifyFile write fVerifyFile;
  ...
  TIdSecSocket = class(TObject)
  private
    fSession: PSSL_SESSION;
    function GetProtocolVersion: TIdSecVersion;
    function GetSSLProtocolVersionStr: string;
    procedure SetSSLContext(AValue: TIdSecContext);
  protected
    {$IFDEF USE_OBJECT_ARC}[Weak]{$ENDIF} fParent: TObject;
    fPeerCert: TIdX509;
    fSSL: PSSL;
    fSSLCipher: TIdSecCipher;
    fSSLContext: TIdSecContext;
    fHostName: String;
    function GetPeerCert: TIdX509;
    function GetSSLCipher: TIdSecCipher;
    procedure SetSessionID(source: TIdSecSocket);
  public
    ...
    //
    property SSLContext: TIdSecContext read fSSLContext write SetSSLContext;
    property PeerCert: TIdX509 read GetPeerCert;
    property Cipher: TIdSecCipher read GetSSLCipher;
    property HostName: String read fHostName write fHostName;
    property SSLProtocolVersion: TIdSecVersion read GetProtocolVersion;
...
begin
  ...
  try
    ...
    CleanupRandom; // <-- RLebeau: why is this here and not in IdSecOpenSSLHeaders.Unload()?
  ...
  try
    ...
    try
      ...
      begin
        ...
        LHelper.StatusInfo(IdSecSocket, where, ret, StatusStr);
        LHelper := nil;
      end;
    finally
      LockInfoCB.Leave;
...
function TranslateInternalVerifyToSSL(Mode: TIdSecVerifyModeSet): Integer;
...
function TIdSecCipher.GetDescription: String;
var
...
function TIdSecCipher.GetName:String;
begin
...
constructor TIdSecContext.Create;
begin
  inherited Create;
...
destructor TIdSecContext.Destroy;
begin
  DestroyContext;
  inherited Destroy;
end;
...
{
function TIdSecContext.LoadVerifyLocations(FileName: String; Dirs: String): Boolean;
begin
  Result := False;
...
function TIdSecContext.LoadRootCert: Boolean;
begin
  Result := IndySSL_CTX_load_verify_locations(fContext, RootCertFile, VerifyDirs) > 0;
end;

function TIdSecContext.LoadCert: Boolean;
begin
...
function TIdSecContext.LoadKey: Boolean;
begin
...
function TIdSecContext.LoadDHParams: Boolean;
begin
  Result := IndySSL_CTX_use_DHparams_file(fContext, fsDHParamsFile, SSL_FILETYPE_PEM) > 0;
end;

function TIdSecContext.IsParent(aObject: TObject): boolean;
begin
...
begin
  Result := nil;
  if fParent <> nil then
    fParent.GetInterface(IIdSecOpenSSLCallbackHelper,Result);
end;
...
constructor TIdSecSocket.Create(Parent: TObject);
begin
  inherited Create;
  fParent := Parent;
end;

destructor TIdSecSocket.Destroy;
begin
  if fSession <> nil then
    SSL_SESSION_free(fSession);
  if fSSL <> nil then begin
    // TODO: should this be moved to TIdSecContext instead?  Is this here
...
function TIdSecSocket.GetSSLError(retCode: Integer): Integer;
begin
  // COMMENT!!!
...
procedure TIdSecSocket.RaiseError(AError: Integer);
begin
  EOpenSSLAPISSLError.RaiseException(fSSL, AError, '');
end;

procedure TIdSecSocket.GetStateVars(AWhere, Aret: TIdC_INT; var VTypeStr,
  VMsg: String);
begin
  case AWhere of
    SSL_CB_ALERT :
...
procedure TIdSecSocket.Accept(const pHandle: TIdStackSocketHandle);
...
procedure TIdSecSocket.Connect(const pHandle: TIdStackSocketHandle);
var
  error: Integer;
  StatusStr: String;
  LHelper: IIdSecOpenSSLCallbackHelper;
  IOHandlerSocket: TIdSecSocket;
begin
  Assert(fSSL=nil);
  Assert(fSSLContext<>nil);
  fSSL := SSL_new(fSSLContext.fContext);
...
function TIdSecSocket.Send(const ABuffer: TIdBytes; AOffset, ALength: Integer): Integer;
var
  ret, err: Integer;
begin
  Result := 0;
...
function TIdSecSocket.GetSSLProtocolVersionStr: string;
begin
...
function TIdSecSocket.GetPeerCert: TIdX509;
var
  LX509: PX509;
begin
...
function  TIdSecSocket.GetSessionIDAsString:String;
var
  Data: TIdSecByteArray;
  i: TIdC_UINT;
  LDataPtr: PByte;
begin
...
procedure TIdSecSocket.SetCipherList(CipherList: String);
//var
//  tmpPStr: PAnsiChar;
begin
{
...
function TIdSecSocket.Readable: TIdSecReadStatus;
var buf : byte;
    Lr: integer;
begin
  Result := sslNoData;
...
procedure TIdSecSocket.DoShutdown;
begin
  if fSSL <> nil then begin
...
function TIdSecSocket.GetSSL: PSSL;
begin
  Result := fSSL;
end;
